...
void DrawConstraint(Vector2 pos, ConstraintType type, float angle,
                    Color color) {
  if (type == CONSTRAINT_FREE)
    return;
...
void DrawInternalConstraint(Vector2 pos, InternalConstraintType type,
                            float angle, Color color) {
  if (type == INT_CONSTRAINT_RIGID)
    return;
  ...
  case INT_CONSTRAINT_ROLLER:
    DrawCircleLines(0, 0, size, color);
    DrawLineEx((Vector2){-size * 1.8f, 0}, (Vector2){size * 1.8f, 0}, 3.0f,
               color);
    break;
  case INT_CONSTRAINT_SLIDER:
    DrawRectangleLinesEx(
        (Rectangle){-size * 1.2f, -size * 0.8f, size * 2.4f, size * 1.6f}, 3.0f,
        color);
    DrawLineEx((Vector2){-size * 1.8f, 0}, (Vector2){size * 1.8f, 0}, 3.0f,
               color);
    break;
...
void DrawForceArrow(Vector2 startPos, Vector2 force, Color color,
  ...
  if (label) {
    DrawText(TextFormat("%s: %.1f", label, mag), endPos.x + 10, endPos.y - 10,
             14, color);
  } else if (showValue) {
    float displayAngle = -angle * RAD2DEG;
    if (displayAngle <= -180.0f)
      displayAngle += 360.0f;
    DrawText(TextFormat("%.0f u | %.0f°", mag, displayAngle), endPos.x + 10,
             endPos.y - 10, 14, color);
  }
}

void DrawBeamDiagram(Vector2 p1, Vector2 p2, float val1, float val2,
...
void DrawDistributedLoads(Node *nodes, Beam *beams, int beamCount) {
  for (int i = 0; i < beamCount; i++) {
    ...
    Vector2 offsetP1 =
        Vector2Add(p1, Vector2Scale(normal, arrow_height * sign));
    Vector2 offsetP2 =
        Vector2Add(p2, Vector2Scale(normal, arrow_height * sign));
    ...
    for (int j = 0; j <= num_arrows; j++) {
      ...
      Vector2 tail =
          Vector2Add(base, Vector2Scale(normal, arrow_height * sign));
    ...
    Vector2 mid = {(offsetP1.x + offsetP2.x) / 2,
                   (offsetP1.y + offsetP2.y) / 2};
    DrawText(TextFormat("q: %.0f N/m", fabs(q)), (int)mid.x - 20,
             (int)mid.y - 20, 15, ORANGE);
  }
}

void DrawParabolicMomentDiagram(Vector2 p1, Vector2 p2, float M_start,
                                float M_end, float q_perp, Color color,
                                float scale) {
...
void DrawTopUI(AppMode mode, float scale, int width) {
  ...
  case MODE_REACTIONS:
    ...
    break;
  case MODE_N:
    DrawText(TextFormat("MODE: [ AXIAL (N) ] - Scale: %.2fx", scale), 20, 10,
             20, COLOR_DIAGRAM_N);
    break;
  case MODE_T:
    DrawText(TextFormat("MODE: [ SHEAR (V) ] - Scale: %.2fx", scale), 20, 10,
             20, COLOR_DIAGRAM_T);
    break;
  case MODE_M:
    DrawText(TextFormat("MODE: [ MOMENT (M) ] - Scale: %.2fx", scale), 20, 10,
             20, COLOR_DIAGRAM_M);
    break;
  }
}
...
void RenderScene(AppScene *scene, InputState *input) {
  ...
  for (int i = 0; i < scene->nodeCount; i++) {
    DrawConstraint(scene->nodes[i].position, scene->nodes[i].constraint,
                   scene->nodes[i].angle, COLOR_EXT_CONSTRAINT);
  }

  for (int i = 0; i < scene->beamCount; i++) {
    Color currentBeamColor =
        (scene->currentMode == MODE_EDIT && i == input->hoveredBeam)
            ? RED
            : COLOR_BEAM;
    DrawLineEx(scene->nodes[scene->beams[i].node_start_idx].position,
               scene->nodes[scene->beams[i].node_end_idx].position, 4.0f,
               currentBeamColor);
  }

  if (input->isDraggingBeam) {
    DrawLineEx(scene->nodes[input->dragStartNodeIdx].position,
               input->snappedPos, 4.0f, (Color){255, 255, 255, 100});
  }

  for (int i = 0; i < scene->nodeCount; i++) {
    DrawInternalConstraint(scene->nodes[i].position,
                           scene->nodes[i].int_constraint,
                           scene->nodes[i].int_angle, COLOR_INT_CONSTRAINT);
    DrawCircleV(scene->nodes[i].position, 5.0f, COLOR_NODE);
    DrawCircleLines(scene->nodes[i].position.x, scene->nodes[i].position.y,
                    5.0f, BLACK);
  }
  ...
  case MODE_EDIT:
    for (int i = 0; i < scene->nodeCount; i++) {
      DrawForceArrow(scene->nodes[i].position, scene->nodes[i].force,
                     COLOR_FORCE, NULL, true);
    }
    if (!input->isDraggingBeam && input->hoveredNodeIdx == -1 &&
        !input->fPressed) {
  ...
  case MODE_REACTIONS:
    for (int i = 0; i < scene->nodeCount; i++) {
      if (fabs(scene->nodes[i].force.x) > 0.1f ||
          fabs(scene->nodes[i].force.y) > 0.1f) {
        DrawForceArrow(scene->nodes[i].position, scene->nodes[i].force,
                       COLOR_FORCE, NULL, true);
      }
      if (scene->nodes[i].constraint != CONSTRAINT_FREE) {
        if (fabs(scene->nodes[i].reaction_force.x) > 0.1f ||
          ...
          DrawForceArrow(scene->nodes[i].position,
                         scene->nodes[i].reaction_force, COLOR_REACTION, "R",
                         false);
        }
        if (fabs(scene->nodes[i].reaction_moment) > 0.1f) {
          DrawRing(scene->nodes[i].position, 20.0f, 24.0f, 45, 315, 32,
                   Fade(COLOR_REACTION, 0.7f));
          DrawText(TextFormat("M: %.0f", scene->nodes[i].reaction_moment),
                   scene->nodes[i].position.x + 28,
                   scene->nodes[i].position.y - 28, 14, COLOR_REACTION);
        }
      }
    }
    break;
  ...
  case MODE_N:
    for (int i = 0; i < scene->beamCount; i++) {
      DrawBeamDiagram(scene->nodes[scene->beams[i].node_start_idx].position,
                      scene->nodes[scene->beams[i].node_end_idx].position,
                      scene->beams[i].N_start, scene->beams[i].N_end,
                      COLOR_DIAGRAM_N, scene->diagramScale);
    }
    break;
  ...
  case MODE_T:
    for (int i = 0; i < scene->beamCount; i++) {
      DrawBeamDiagram(scene->nodes[scene->beams[i].node_start_idx].position,
                      scene->nodes[scene->beams[i].node_end_idx].position,
                      scene->beams[i].T_start, scene->beams[i].T_end,
                      COLOR_DIAGRAM_T, scene->diagramScale);
    }
    break;
  ...
  case MODE_M:
    for (int i = 0; i < scene->beamCount; i++) {
      DrawParabolicMomentDiagram(
          ...
          scene->nodes[scene->beams[i].node_end_idx].position,
          scene->beams[i].M_start, scene->beams[i].M_end,
          scene->beams[i].q_perp, COLOR_DIAGRAM_M, scene->diagramScale);
    }
    break;
  }
}
...
void DrawBackgroundGrid(void) {
  ...
  for (int i = -numLines; i <= numLines; i++) {
    DrawLineV((Vector2){i * GRID_SIZE, -numLines * GRID_SIZE},
              (Vector2){i * GRID_SIZE, numLines * GRID_SIZE}, COLOR_GRID);
    DrawLineV((Vector2){-numLines * GRID_SIZE, i * GRID_SIZE},
              (Vector2){numLines * GRID_SIZE, i * GRID_SIZE}, COLOR_GRID);
  }
}