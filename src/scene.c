...
int GetNodeAtGridPosition(Node *nodes, int count, Vector2 pos) {
	for (int i = 0; i < count; i++)
		if (Vector2Distance(nodes[i].position, pos) < 1.0f) return i;
	return -1;
}

int GetNodeUnderMouse(Node *nodes, int count, Vector2 mouseWorldPos) {
	for (int i = 0; i < count; i++)
		if (Vector2Distance(nodes[i].position, mouseWorldPos) < 15.0f) return i;
	return -1;
}
...
void UpdateInputState(InputState *input, AppScene *scene, Camera2D camera) {
	...
	input->snappedPos = (Vector2){
		roundf(input->mouseWorldPos.x / GRID_SIZE) * GRID_SIZE,
	...
	input->hoveredBeam =
		GetHoveredBeam(input->mouseWorldPos, scene->nodes, scene->beams, scene->beamCount);
...
void HandleForceDragging(AppScene *scene, InputState *input) {
	...
	if (input->isDraggingForce) {
		Vector2 rawForce =
			Vector2Subtract(input->mouseWorldPos, scene->nodes[input->forceNodeIdx].position);
		...
		input->wheel != 0 && input->hoveredNodeIdx != -1 &&
		Vector2Length(scene->nodes[input->hoveredNodeIdx].force) > 0) {
		float mag = Vector2Length(scene->nodes[input->hoveredNodeIdx].force);
		float ang = atan2f(
			scene->nodes[input->hoveredNodeIdx].force.y,
			scene->nodes[input->hoveredNodeIdx].force.x);
...
void HandleNodeConstraints(AppScene *scene, InputState *input) {
	if (IsMouseButtonPressed(MOUSE_BUTTON_LEFT) && input->hoveredNodeIdx != -1) {
		if (input->shiftPressed) {
			scene->nodes[input->hoveredNodeIdx].int_constraint =
				(scene->nodes[input->hoveredNodeIdx].int_constraint + 1) % INT_CONSTRAINT_COUNT;
			if (scene->nodes[input->hoveredNodeIdx].int_constraint == INT_CONSTRAINT_RIGID) {
				scene->nodes[input->hoveredNodeIdx].int_angle = 0.0f;
			}
		} else {
			scene->nodes[input->hoveredNodeIdx].constraint =
				(scene->nodes[input->hoveredNodeIdx].constraint + 1) % CONSTRAINT_COUNT;
	...
	if (input->wheel != 0 && input->hoveredNodeIdx != -1) {
		if (input->shiftPressed)
			scene->nodes[input->hoveredNodeIdx].int_angle += input->wheel * 15.0f;
		else
			scene->nodes[input->hoveredNodeIdx].angle += input->wheel * 15.0f;
		input->wheel = 0;
	}
}
...
void ProcessAppInput(AppScene *scene, InputState *input, Camera2D *camera) {
	...
	if (IsMouseButtonDown(MOUSE_BUTTON_MIDDLE)) {
		camera->target =
			Vector2Add(camera->target, Vector2Scale(GetMouseDelta(), -1.0f / camera->zoom));
	}
