...
static void GetBeamReleases(
	int c1, int c2, bool *rNs, bool *rTs, bool *rMs, bool *rNe, bool *rTe, bool *rMe) {
	*rNs = *rTs = *rMs = *rNe = *rTe = *rMe = false;

	if (c1 == INT_CONSTRAINT_HINGE)
		*rMs = true;
	else if (c1 == INT_CONSTRAINT_SLEEVE)
		*rNs = true;
	else if (c1 == INT_CONSTRAINT_SLIDER)
		*rTs = true;
	else if (c1 == INT_CONSTRAINT_ROLLER) {
		*rMs = true;
		*rNs = true;
	}

	if (c2 == INT_CONSTRAINT_HINGE)
		*rMe = true;
	else if (c2 == INT_CONSTRAINT_SLEEVE)
		*rNe = true;
	else if (c2 == INT_CONSTRAINT_SLIDER)
		*rTe = true;
	else if (c2 == INT_CONSTRAINT_ROLLER) {
		*rMe = true;
		*rNe = true;
	}
}

static void GetElementLocalMatrices(
	double L,
	double E,
	double A_sez,
	double I_sez,
	...
	if (fabs(q_p) > 1e-5 || fabs(q_a) > 1e-5) {
		if (!rNs && !rNe) {
			f_eq_loc[0] = -(q_a * L) / 2.0;
			f_eq_loc[3] = -(q_a * L) / 2.0;
		}
		if (!rMs && !rMe) {
			f_eq_loc[1] = -(q_p * L) / 2.0;
			f_eq_loc[2] = -(q_p * L * L) / 12.0;
			f_eq_loc[4] = -(q_p * L) / 2.0;
			f_eq_loc[5] = (q_p * L * L) / 12.0;
		} else if (rMs && !rMe) {
			f_eq_loc[1] = -(3.0 / 8.0 * q_p * L);
			f_eq_loc[4] = -(5.0 / 8.0 * q_p * L);
			f_eq_loc[5] = (q_p * L * L) / 8.0;
		} else if (!rMs && rMe) {
			f_eq_loc[1] = -(5.0 / 8.0 * q_p * L);
			f_eq_loc[2] = -(q_p * L * L) / 8.0;
			f_eq_loc[4] = -(3.0 / 8.0 * q_p * L);
		} else {
			f_eq_loc[1] = -(q_p * L) / 2.0;
			f_eq_loc[4] = -(q_p * L) / 2.0;
		}
...
static void GetTransformationMatrix(double c, double s, double T[6][6]) {
	double temp[6][6] = {
		...
		{0, 0, 0, 0, 0, 1}};
	for (int i = 0; i < 6; i++)
		for (int j = 0; j < 6; j++) T[i][j] = temp[i][j];
}
...
void SolveFEM(Node *nodes, int nodeCount, Beam *beams, int beamCount) {
	...
	for (int b = 0; b < beamCount; b++) {
		...
		GetElementLocalMatrices(
			...
			beams[b].q_perp,
			beams[b].q_axial,
			nodes[n1].int_constraint,
			nodes[n2].int_constraint,
			k_loc,
			f_eq_loc);
		...
		for (int i = 0; i < 6; i++)
			for (int j = 0; j < 6; j++)
				for (int k = 0; k < 6; k++) temp[i][j] += k_loc[i][k] * T[k][j];
		for (int i = 0; i < 6; i++)
			for (int j = 0; j < 6; j++)
				for (int k = 0; k < 6; k++) k_global_el[i][j] += T[k][i] * temp[k][j];
	...
	if (!SolveLinearSystem(K_global, F_global, numDOFs)) {
		for (int i = 0; i < beamCount; i++)
			beams[i].N_start = beams[i].N_end = beams[i].T_start = beams[i].T_end =
				beams[i].M_start = beams[i].M_end = 0;
		free(K_global);
		free(K_orig);
		free(F_global);
		free(F_orig);
	...
	for (int b = 0; b < beamCount; b++) {
		...
		for (int i = 0; i < 6; i++)
			for (int j = 0; j < 6; j++) u_loc[i] += T[i][j] * u_glob[j];
		for (int i = 0; i < 6; i++) {
			for (int j = 0; j < 6; j++) f_loc_b[i] += k_loc[i][j] * u_loc[j];
			f_loc_b[i] -= f_eq_loc[i];	// Sottraiamo le forze nodali equivalenti!
		}

		beams[b].N_start = (float)SnapToZero(-f_loc_b[0], zero_tol);
		beams[b].N_end = (float)SnapToZero(f_loc_b[3], zero_tol);
		beams[b].T_start = (float)SnapToZero(f_loc_b[1], zero_tol);
		beams[b].T_end = (float)SnapToZero(-f_loc_b[4], zero_tol);
		beams[b].M_start = (float)SnapToZero(-f_loc_b[2], zero_tol);
		beams[b].M_end = (float)SnapToZero(f_loc_b[5], zero_tol);
	}
